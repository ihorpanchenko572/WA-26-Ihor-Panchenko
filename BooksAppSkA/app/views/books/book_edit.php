    ...
    <div class="relative">
        <select id="subcategory" name="subcategory" 
            ...
            <?php foreach ($subcategories as $sub): ?>
                <?php 
                // Logika pro předvýběr: porovnáme ID z cyklu s ID v databázi u knihy
                $isSubSelected = ($book['subcategory'] == $sub['id']) ? 'selected' : ''; 
                ?>
                
                <option value="<?= htmlspecialchars($sub['id']) ?>" <?= $isSubSelected ?> class="bg-slate-900 text-slate-200">
        ...
        <!-- Oranžová šipka pro select -->
        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-orange-500/50">
